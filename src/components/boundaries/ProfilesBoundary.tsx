import { profileStoreSync } from "@/stores/profiles";
...
export function ProfilesBoundary(props: Props) {
  const { t } = useTranslation();
  const { hasLoaded, isPending, error } = useSyncLoadState(profileStoreSync);
  if (error) {
    return (
      <PageContainer>
        ...
        <div>
          <PageAlert>{t("errors.profilesLoadFailed")}</PageAlert>
